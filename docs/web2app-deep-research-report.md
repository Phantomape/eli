...
- Apple 的 AdAttributionKit 进一步确认了 iOS 侧的大方向：广告归因会更多依赖隐私阈值、延迟 postback、转化值和已注册广告网络，而不是用户级确定性追踪。
...
- `Playable App Ads` 等互动格式更适合“先试后装”的游戏和工具类 App，尤其适合把试玩、预览和安装意图筛选前置到广告内
- `Sponsored Snaps`、App End Cards、tCPA 等能力更偏向提高 App 安装广告的触达、点击和成本控制
